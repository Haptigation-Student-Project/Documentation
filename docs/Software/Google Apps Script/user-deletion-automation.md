...
Email format is validated: `[text]@[domain].[tld]`
Check for typos
...
### Permanently delete emails instead of Trash
**Default:** Emails are moved to Trash
**For permanent deletion**, replace in `deleteEmailsByAddress()`:
```javascript
thread.moveToTrash();
```
with:
```javascript
GmailApp.emptyTrash(); // CAUTION: Deletes the entire Trash!
```
**CAUTION:** This deletes ALL emails in the Trash, not just the user's!
...
Administrator sets email in CONFIG
Runs `deleteByEmail()`
User receives automatic confirmation
Deletion process is logged (compliance proof)
### Former employees
Set the employee's email address in CONFIG
Execute deletion process
All business emails remain in Trash (restorable if needed)
Contacts are permanently deleted
### Clean up test data
Collect test email addresses
Delete sequentially or via batch function
Run regularly after tests
Keeps Gmail and Contacts clean
...
- CC and BCC are NOT directly captured
**Batch processing:**
- Threads are processed in batches (default: 500)
- Prevents timeout with large volumes
- Automatically continues until all threads are processed
### Contact search
**People API:**
- Searches all Google Contacts
- Case-insensitive comparison
- Finds contacts with multiple email addresses
**Deletion process:**
- Permanent (not recoverable)
- 300ms wait between deletions
- Fault tolerance: continues on error
### Performance
**Speed:**
- ~500 email threads: 1-2 minutes
- ~50 contacts: 30-60 seconds
- Varies by account size and Gmail load
**Limits:**
- Script timeout: 6 minutes
- Gmail API: rate limits may apply
- For large volumes: multiple runs required
...
- Starts 3-second warning
- Calls `deleteAllDataForEmail()`
**deleteAllDataForEmail(email)**
...
- Deletes contacts individually
- Returns count
**deleteEmailsByAddress(email)**
...
**anonymizeEmail(email, partial)** - Anonymizes emails for logs
**log(level, message)** - Central logging with timestamp
**showLogs()** - Shows all logs
